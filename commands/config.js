...
async function showConfig(interaction) {
    try {
        ...
        const embed = new EmbedBuilder()
            .setTitle('âï¸ Bot Configuration')
            .setColor(0x00AE86)
            .addFields(
                {
                    name: 'ð Suggestions System',
                    value: `Enabled: ${config.suggestions.enabled ? 'â' : 'â'}
                ...
                {
                    name: 'ð Scheduler System',
                    value: `Enabled: ${config.scheduler.enabled ? 'â' : 'â'}
...
async function configureSuggestions(interaction) {
    try {
        ...
        if (updated) {
            ...
            await interaction.reply({ content: 'â Suggestions configuration updated successfully!', flags: 64 });
        } else {
...
async function configureScheduler(interaction) {
    try {
        ...
        if (updated) {
            fs.writeFileSync(configPath, JSON.stringify(config, null, 4));
            await interaction.reply({ content: 'â Scheduler configuration updated successfully!', flags: 64 });
        } else {
            await interaction.reply({ content: 'No changes were made to the configuration.', flags: 64 });
        }
    } catch (error) {
...
async function configureRoles(interaction) {
    try {
        ...
        if (updated) {
            fs.writeFileSync(configPath, JSON.stringify(config, null, 4));
            await interaction.reply({ content: 'â Roles configured successfully using role IDs!', flags: 64 });
        } else {
