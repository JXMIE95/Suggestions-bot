...
async function handleSuggestionSubmit(interaction) {
    try {
        ...
        const embed = new EmbedBuilder()
            ...
            .setColor(0x00AE86)
            .addFields(
                { name: 'ðð» Upvotes', value: '0', inline: true },
                { name: 'ð Downvotes', value: '0', inline: true },
        ...
        await interaction.reply({ content: 'â Your suggestion has been submitted successfully!', ephemeral: true });
    } catch (error) {
    ...
    try {
        ...
        db().get(
            ...
            async (err, suggestion) => {
                ...
                // Count reactions
                const upvoteReaction = message.reactions.cache.get('ðð»');
                const downvoteReaction = message.reactions.cache.get('ð');
...
async function sendToStaffChannel(client, suggestion, upvotes, downvotes) {
    try {
        ...
        const embed = new EmbedBuilder()
            .setTitle('ð Staff Vote Required')
            .setDescription(suggestion.content)
            .setColor(0xFFAA00)
            .addFields(
                { name: 'Community Votes', value: `ðð» ${upvotes} | ð ${downvotes}`, inline: false },
...
async function handleStaffVote(interaction) {
    try {
        ...
        await interaction.reply({ content: `â Your ${voteType === 'approve' ? 'approval' : 'rejection'} vote has been recorded.`, ephemeral: true });
...
async function announceResult(client, suggestion, approved, approvals, rejections) {
    try {
        ...
        const embed = new EmbedBuilder()
            .setTitle(approved ? 'â Suggestion Approved!' : 'â Suggestion Rejected')
            .setDescription(suggestion.content)
            .setColor(approved ? 0x00FF00 : 0xFF0000)
            .addFields(
                { name: 'Community Votes', value: `ðð» ${suggestion.upvotes} | ð ${suggestion.downvotes}`, inline: true },
                { name: 'Staff Decision', value: `â ${approvals} | â ${rejections}`, inline: true }
            )
            .setTimestamp()
