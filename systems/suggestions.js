...
async function postSuggestionButton(client) {
    try {
        const config = JSON.parse(fs.readFileSync(configPath, 'utf8'));
        if (!config.suggestions.enabled || !config.suggestions.submitChannelId) return;

        const channel = await client.channels.fetch(config.suggestions.submitChannelId);
        if (!channel) {
            logger.warn('Submit channel not found');
            return;
        }
        ...
        const alreadyExists = messages.find(msg =>
            ...
            msg.components[0].components.some(btn => btn.customId === 'submit_suggestion')
        );
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription('Click the button below to share your idea with the community!')
            .setColor(0x00AE86);
