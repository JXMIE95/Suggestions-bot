...
async function sendShiftNotification(client, shift, date) {
    try {
        ...
        const kingRole = await guild.roles.fetch(config.scheduler.kingRoleId).catch(() => null);
        const buffRole = await guild.roles.fetch(config.scheduler.buffGiverRoleId).catch(() => null);

        let mentions = '';
        if (kingRole) mentions += `<@&${kingRole.id}> `;
        if (buffRole) mentions += `<@&${buffRole.id}>`;
