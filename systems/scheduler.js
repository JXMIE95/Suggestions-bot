    ...
    try {
        ...
        for (const slot of timeSlots) {
            const fieldValue = slot.users.length > 0 
                ? slot.users.map(user => `â¢ ${user.username} (${user.role})`).join('\n')
                : 'No one scheduled';
        ...
        db().all(
            ...
            async (err, occupiedSlots) => {
                ...
                for (let hour = 0; hour < 24; hour++) {
                    ...
                    const label = `${timeString} UTC ${available ? 'â' : 'â (Full)'}`;
                ...
                const embed = new EmbedBuilder()
                    .setTitle('ð Add Your Availability')
                    .setDescription(`**Select multiple time slots for ${date}**\n\n` +
                        '1ï¸â£ Choose one or more time slots from the list below\n' +
                        '2ï¸â£ Click the "Confirm Selection" button\n\n' +
                        'â = Available slots\n' +
                        'â = Full slots (2/2 people already scheduled)\n\n' +
                    ...
                    async (err, slots) => {
                        ...
                        db().run(
                            ...
                            async function(err) {
                                ...
                                await interaction.reply({ content: `â Added to roster at ${timeSlot} as ${roleType}`, ephemeral: true });
                            }
                        );
                    }
                );
...
async function handleRosterTimeSelect(interaction) {
    try {
        ...
        await interaction.reply({ 
            content: `â Selected ${interaction.values.length} time slot(s): ${interaction.values.join(', ')}\n` +
...
async function handleRosterConfirm(interaction) {
    try {
        ...
        if (successfulSlots.length > 0) {
            response += `â Successfully added to roster:\n${successfulSlots.map(slot => `â¢ ${slot} as ${selection.role}`).join('\n')}`;
        }
        if (failedSlots.length > 0) {
            response += `\n\nâ Failed to add:\n${failedSlots.map(slot => `â¢ ${slot}`).join('\n')}`;
        }
...
async function handleRosterCancelSelection(interaction) {
    try {
        ...
        await interaction.reply({ 
            content: 'â Selection cancelled. Your choices have been cleared.', 
            flags: 64 
        });
    } catch (error) {
            ...
            async (err, entries) => {
                ...
                db().run(
                    ...
                    async (err) => {
                        ...
                        await updateRosterMessage(interaction.channel, date);
                        await interaction.reply({ content: `â Canceled all your availability for ${date}`, ephemeral: true });
                    }
                );
            }
        );
            ...
            (err) => {
                ...
                db().run(
                    ...
                    async (err) => {
                        ...
                        await updateRosterMessage(interaction.channel, date);
                        await interaction.reply({ content: `â Updated availability to ${newTimeSlot} as ${newRoleType}`, ephemeral: true });
                    }
                );
            }
        );
            ...
            async (err, roster) => {
                ...
                for (const [time, users] of Object.entries(timeSlots)) {
                    const fieldValue = users.length > 0 
                        ? users.map(user => `â¢ ${user.username} (${user.role})`).join('\n')
                        : 'No one scheduled';
    ...
    try {
        ...
        db().all(
            ...
            async (err, users) => {
                ...
                const embed = new EmbedBuilder()
                    .setTitle('â° Shift Starting Soon!')
                    ...
                    .addFields(
                        ...
                        { name: 'Scheduled Users', value: users.map(u => `â¢ ${u.username} (${u.roleType})`).join('\n'), inline: false }
                    )
                    .setTimestamp();
    ...
    try {
        ...
        db().run(
            ...
            async (err) => {
                ...
                await interaction.reply({ content: `â Checked in for ${timeSlot} UTC shift on ${date}`, ephemeral: true });
            }
        );
    } catch (error) {
